...
if (encrypting):
    ...
    if whichcipher == "caesar":
        keyrotate = 0
        plaintext = ""
        skipped = 0
        for i in range(len(ciphertext)):
            p = ciphertext[i] - ord('A')
            c = chr(ord('A') + ((p - keyrotate) % 26))
            plaintext += c
        else:
            skipped += 1
        ...
        output.close
    elif whichcipher == "julia":
        keyshift = 1
        keyxor = []
        keyxorasstring = "jKCLjJjbkrsE"

        plaintext = []

        for i in range(len(keyxorasstring)):
            keyxor.append(ord(keyxorasstring[i]))
        print(keyxor)
        for i in range(len(ciphertext)):
            n = ciphertext[i]
            d = 2
            print(ciphertext[i], lrot(rrot(ciphertext[i], keyshift), keyshift))
